...
router.post("/books", (req, res) => {
    const { title, author, description, copies, imageUrl } = req.body;
    db.run("INSERT INTO books (title, author, description, copies, imageUrl) VALUES (?, ?, ?, ?, ?)", [title, author, description, copies, imageUrl], function(err){
        if (err) {
...
router.put('/books/:id', (req, res) => {
  const { id } = req.params;
  const { title, author, description, copies, imageUrl } = req.body;
  db.run('UPDATE books SET title = ?, author = ?, description = ?, copies = ?, imageUrl = ? WHERE id = ?', [title, author, description, copies, imageUrl, id], function(err) {
      if (err) {
          res.status(400).json({ error: err.message });
          return;
      }
